...
var showCss = function(fontFullPath, css , cb ) {
    cb(null, css);
};
...
function getCSS( name , output, base64, showContent, cb) {
    console.log(name + " - " + output + " - " + base64 + " - " + showContent)
    ...
    async.series([
        ...
        function(callback) {
            if ( !base64 ) {
                if(!showContent ) ftpm.writeFile(fontFullPath , css , callback);
                 else showCss(fontFullPath , css , callback);
            } else {
        ...
        function(callback){
            ftpm.file.readFile( tmpFile , 'base64', function( err, content ) {
                ...
                ftpm.file.unlink( tmpFile , function(err) {
                    if (err) callback(err);
                    if(!showContent ) ftpm.writeFile(fontFullPath , content , callback);
                    else showCss(fontFullPath , content , callback);
                });
            });
        }
    ], cb);
